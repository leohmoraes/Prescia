...
Esta seção é a referência vigente para o estado do repositório. A análise foi executada localmente sobre a árvore correspondente ao `master` após o merge da documentação no commit `c262bd4f5b8a5ec4058e4af2839853f4cba1f786`, usando PHP 8.3.6, Composer com o lockfile versionado e a configuração oficial `phpstan.neon.dist`.

| Verificação | Resultado | Evidência |
|---|---|---|
| PHPStan global | **Aprovado** | PHPStan 2.2.13, nível 1, `vendor/bin/phpstan analyse --configuration=phpstan.neon.dist --no-progress --error-format=json`, `0 errors`, `0 file_errors` |
...
O projeto mantém uma análise estática incremental com **PHPStan 2.x no nível 1**, baseline sem supressões novas e contratos explícitos para os principais contextos dinâmicos do framework. Na execução de 2026-09-14, a análise global, o PHPUnit e o lint foram executados localmente em PHP 8.3.6 com resultado aprovado, com duas deprecações e dois testes pulados já registrados pela configuração.
...
As contagens de centenas de diagnósticos e o limite histórico de 1.000 registros pertencem a execuções anteriores. Nesta execução, o formatter JSON oficial retornou zero erros; a validação futura deve repetir a mesma configuração e registrar o SHA analisado antes de comparar resultados.
...
| PHPStan completo do repositório | **Aprovado** | PHPStan 2.2.13, nível 1, `phpstan.neon.dist`, JSON sem erros no commit analisado |
