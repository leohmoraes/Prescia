...
class ttree {
	...
	# Gets an array with 'id', '$idParField' and '$titleField', and get them into a tree structure
	# This will call fillme, except it will fill a "tree" tag with the path to each item based on treestrsep and titleField
	# I you don't need this tree structure, just call fillme
	public function arrayToTree($array,$treestrsep = "\\",$idParField='id_parent',$titleField='treetitle',$removeSelfFromTitle=false) {
		# First, get all items in a translator ID => actual position in the array
		$translator = array();
		...
		$x = 0;
		foreach ($array as $id => $content) {
			$array[$id]['tree'] = $removeSelfFromTitle?'':$array[$id][$titleField];
			$thisitem = $content;
			if ($thisitem[$idParField] != 0) {
				while ($thisitem[$idParField] != 0 && !is_null($thisitem[$idParField])) {
					...
					$array[$id]['tree'] = $array[$translator[$thisitem[$idParField]]][$titleField].$treestrsep.$array[$id]['tree'] ;
					$x++;
					if ($x == 1000) die("Overflow at tree builder (infinite loop? op:1000)");
	...
	## fillme: Gets an array of data and build the tree, where each item MUST have at least:
	##	id' and 'id_parent'.
	##  me is the selected branch or false (none selected)
	private function fillme($array, $rootID = false,$root = true) {
		...
		foreach ($this->branchs as $x => $branch) { // fills up children
	...
	public function echoHTML($ignoreMyTag=false) {
		...
		if ($this->data[0] != '' && !$ignoreMyTag) {
			...
			if (is_array($this->data[1])) {
				...
				$output .= " ".$this->data[1];
			if (in_array($tag,$autoclose) && $this->data[2] == '' && count($this->branchs == 0)) $output .= "/"; // do not auto-close tags with content
			$output .= ">";
		}
		$output .= $this->data[2];
		foreach ($this->branchs as $branch) {
		...
		if (!$ignoreMyTag && ($this->data[0] != '' || count($this->branchs > 0)) && substr($tag,0,8) != '![cdata[' && substr($tag,0,8) != '![CDATA[')
			$output .= "</".$tag.">";
		return $output;
	}
