...
define ("C_XHTML_SIMPLE","B,I,EM,STRONG,UL,LI,OL,DIV,P,SPAN,BLOCKQUOTE,BR,HR,H1,H2,H3,H4,H5,H6,CODE,KBR,PRE"); # these are the only allowed tags on simple HTML formating (which also strips parameters from them)
...
function parseHTML($html,$simplify=false) {
	...
	$xObj = new xmlHandler();
	$html = $xObj->parseXML($html,array(C_XML_LAX => true,C_XML_REMOVECOMMENTS => true),false,true); // don't parse parameters, silent
	if ($html!==false) {
...
class xmlHandler {
  ...
  private $xhtmlcode = array(); # processed C_XHTML_CODE for faster search
  private $xhtmlsimple = array(); # processed C_XHTML_SIMPLE for faster search
  ...
  public function __construct() {
	...
	unset($xhtmlat);
	# pre-process C_XHTML_SIMPLE for faster parser:
	$xhtmlat = explode(",",C_XHTML_SIMPLE);
	$this->xhtmlsimple = array();
	foreach ($xhtmlat as $tab)
		$this->xhtmlsimple[strtoupper($tab)] = true;
	unset($xhtmlat);
  ...
  public function parseXML($data,$params=array(),$fetchData=false,$silent=false) {
			...
			if ($buffer[0] == "!" || $buffer[0] == "?") { // stand-alone comment tag, consider it auto-close but don't parse
				...
				if (!$params[C_XML_RAW] && isset($this->xhtmlacl[$utag])) { # this should be a auto-close tag!
					if ($laxClosure) {
						...
						if (!$silent) echo "XML: Auto-close tag not closed at line $line: $tag";
						return false;
					}
				} else {
					$buffer = trim(substr($buffer,strlen($tag)+1));
	...
	if (!$laxClosure && count($intags)>0 && !$silent) echo "XML: Tag not closed at ".array_pop($intags); # this should never happen on LAX mode
	return $saida;
  }
  ...
  public function outputHTML($htmltree,$autotab = true, $compact = false, $simplify = false) {
  	$output = "";
  	if (count($htmltree->data) ==0) {
  		$branch = $htmltree->branchs[0]; # this should be the 0,0,xhtml tag generated by the parseXML
  		$branch->data[0] == ""; # removes the xhtml auto-tag
	} else {
  		$branch = $htmltree;
	}
  	$justcrlf = false;
  	$output = $this->outputHTML_ex($branch,$autotab,$compact,$simplify,0,$justcrlf);
  	return $output;
  }

  private function outputHTML_ex($branch,$autotab,$compact,$simplify,$tlevel,&$justcrlf) {
  	# simplify won't echo parameters, and won't display tags other than C_XHTML_SIMPLE
  	$output = "";
  	$prefix = "";
  	$breaker = $compact?"":"\n";
	$justcrlf = true;
  		...
  		if (count($childbranch->branchs) == 0) { # no childs, thus a simple tag
			if ($childbranch->data[2] != "") { # we have raw TEXT to output
				if ($childbranch->data[0] != "") { # such raw TEXT is inside a TAG
					$oktogo = !$simplify || isset($this->xhtmlsimple[$childbranch->data[0]]);
					$output .= $oktogo?(($justcrlf?$prefix:"")."<".strtolower($childbranch->data[0]).($childbranch->data[1] != "" && !$simplify?" ".$this->tostr($childbranch->data[1]):"").">"):"";
					$output .= $childbranch->data[2];
					$output .= $oktogo?"</".strtolower($childbranch->data[0]).">":'';
					$justcrlf = false;
				...
				if ($childbranch->data[0][0] == "?" || $childbranch->data[0][0] == "!") {# xml or php				
					$output .= !$simplify ? ("<".strtolower($childbranch->data[0]).($childbranch->data[1] != ""?" ".$this->tostr($childbranch->data[1]):"").($childbranch->data[0][0] == "?"?" ?":"").">$breaker"):$breaker;
					$justcrlf = true;
					...
					if ($oktogo) {
						...
						$output .= ($justcrlf?$prefix:"")."<".strtolower($childbranch->data[0]).($childbranch->data[1] != "" && !$simplify?" ".$this->tostr($childbranch->data[1]):"")." />"; ## TODO: check alts on image
  			...
  			if ($autotab && isset($this->xhtmltag[strtoupper($childbranch->data[0])]) && $oktogo) {
  				...
  				$tlevel++;
  				$justcrlf = true;
  				$output .= $this->outputHTML_ex($childbranch,$autotab,$compact,$simplify,$tlevel,$justcrlf);
  				$tlevel--;
  				if (!$justcrlf) $output .= $breaker;
  				$justcrlf = true;
  			} else {
  				$output .= ($justcrlf?$prefix:"").($oktogo?("<".strtolower($childbranch->data[0]).($childbranch->data[1] != "" && !$simplify?" ".$this->tostr($childbranch->data[1]):"").">"):'');
  				$justcrlf = false;
  				$output .= $this->outputHTML_ex($childbranch,$autotab,$compact,$simplify,$tlevel,$justcrlf);
  			}
  			$output .= ($justcrlf?$prefix:"").($oktogo?("</".strtolower($childbranch->data[0]).">".$breaker):'').$childbranch->data[2];
  			$justcrlf = true;
  		}
  	}
  	return $output;
